...
public sealed class CloneTests(ITestOutputHelper testOutput) : IDisposable
{
    ...
    public void CloneTautRepo0IntoRepo2()
    {
        ...
        _plan.RunGitOnRoot("clone", "--origin", Repo0, $"taut::{Repo0}", Repo2);

        UpdateContentAndPush(_plan.Repo2Root);

        using var repo2 = Lg2Repository.New(_plan.Repo2Root);
        var repo0SiteName = repo2.FindTautSiteNameForRemote(Repo0);

        _plan.RunGitOnRepo1("pull");
    ...
    public void CloneTautRepo0IntoRepo2_ViaHttp()
    {
        ...
        _plan.RunGitOnRoot("clone", "--origin", Repo0, $"taut::{repo0HttpUri.AbsoluteUri}", Repo2);
    ...
    public void CloneTautRepo0IntoRepo2_ViaSsh()
    {
        ...
        _plan.RunGitOnRoot("clone", "--origin", Repo0, tautRepo0Sshuri, Repo2);
    ...
    public void CloneTautRepo0IntoRepo2_WithTags()
    {
        ...
        _plan.RunGitOnRepo2("tag", "tag1", "HEAD");
    ...
    public void AddTautRepo0IntoRepo2()
    {
        ...
        _plan.RunGitOnRoot("init", Repo2);

        _plan.RunGitOnRepo2("taut", "add", Repo0, Path.Join("..", Repo0));

        using var repo2 = Lg2Repository.New(_plan.Repo2Root);

        _plan.RunGitOnRepo2("pull", Repo0, Master);
    ...
    public void AddTautRepo1IntoRepo2_WithLinkToRepo0()
    {
        ...
        using var repo2 = Lg2Repository.New(_plan.Repo2Root);
        var repo2SiteRepo9Name = repo2.FindTautSiteNameForRemote(Repo9);

        var repo2SiteRepo9Root = GitRepoHelpers.GetTautSitePath(
            _plan.Repo2GitDir,
            repo2SiteRepo9Name
        );
        using var repo2SiteRepo9 = Lg2Repository.New(repo2SiteRepo9Root);
        using var repo9Base = Lg2Repository.New(_plan.Repo9Root);

        AssertSameBranchId(repo9Base, repo2SiteRepo9, Master);
        AssertSiteHeadRef(repo2SiteRepo9);

        UpdateContentAndPush(_plan.Repo2Root);

        _plan.RunGitOnRepo2("push", Repo9);
    }
}
